...
namespace video {
		...
		void base_shader_buffers::free(base_shader_buffers *p_buffer) noexcept
		{
			if (p_buffer)
			{
				GLuint l_model = std::exchange(p_buffer->model[0], {});
				GLuint l_frame = std::exchange(p_buffer->frame[0], {});
				GLuint l_blend = std::exchange(p_buffer->blend[0], {});
