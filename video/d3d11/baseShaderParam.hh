#ifndef SHAPE_CORE_VIDEO_D3D11_BASESHADERPARAM_HH__GUARD
#define SHAPE_CORE_VIDEO_D3D11_BASESHADERPARAM_HH__GUARD
...
namespace shape
{
	namespace video
	{
		template<class Concrete> class CoreVideoBaseShaderParam : private crtp<Concrete>
		{
		public:
			...
			inline CoreVideoBaseShaderParam(D3D11Device p_device)
			{
		...
		template<class Concrete> void CoreVideoBaseShaderParam<Concrete>::attach(D3D11DeviceContext p_context)
		{
		...
		template<class Concrete> void CoreVideoBaseShaderParam<Concrete>::detach(D3D11DeviceContext p_context)
		{
			ID3D11Buffer * const l_buffers[] =
			{
				nullptr,
